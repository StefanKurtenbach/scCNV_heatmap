    ...
    for bed_file in bed_files:
        with open(bed_file) as b:
            for line in b:
                ...
                if line_split[0] != "":
                    if line_split[0][:3] == "chr" or line_split[0][:3] == "Chr":
                        line_split[0] = int(line_split[0][3:])
                    else:
                ...
                if region_to_draw != [0, 0]:
                    write_to_file(draw_rect(x_start + (((region_to_draw[0] - region[1]) * total_width) / float((region[2] - region[1]))), y_position_bed - 5.3 + (6 / 2) + 5, "#0B34FF", ((region_to_draw[1] - region_to_draw[0]) * 150) / float(region[2] - region[1]), 6, 1))
        y_position_bed += 10
